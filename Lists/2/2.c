// Após fazer uma busca por valor, insira ordenado uma lista. Ao digitar um
// valor, o seu código deve fazer a inserção ordenada.
...
void InsertValue(knot **beg) {
  ...
  if ((new = (knot *)malloc(sizeof(knot))) == NULL) {
    ...
    new->val = val;
    if (*beg == NULL) { // se a lista estiver vazia
      new->nxt = NULL;
      *beg = new;
    } else {
      while (read->nxt != NULL) {
...
void pileUp(knot **beg, int val) {
  ...
  if ((new = (knot *)malloc(sizeof(knot))) == NULL) // aloca memória e verifica se houve êxito nessa operação
    ...
    if (*beg == NULL) // verifica se é o último  (ou, no caso, o único?) item da pilha
      new->nxt = NULL; // se sim, salva o ponteiro como NULL, indicando que ele
    // é o último item.
    else
...
// void pileInBtwn(
//     knot **beg,
//     int val) { // i know it's not that way, but you get what I mean...
//   knot *new, *read = *beg;
//   int pos = 0;
//   if ((new = (knot *)malloc(sizeof(knot))) == NULL) {
//     printf("Memory Full!\n");
...
// }
//
// void pileDown(knot **beg, int val) { // i know it's not that way, but you get
// what I mean...
//   knot *new, *read = *beg; //?
//   if ((new = (knot *)malloc(sizeof(knot))) == NULL) {
//     printf("Memory Full!\n");
...
void srchStack(knot *beg) {
  ...
  if (beg == NULL) {
    printf("The stack is empty!\n");
    found = 1; // no caso, found nothing, mas eu não consegui pensar numa lógica
               // melhor...
  } else {
    getVal(&val);
    do {
